...
impl Timer {
    ...
    ///
    /// This will always be clamped to a minimum of 1 microsecond. Giving a duration of 0 will
    /// result in a timer with a duration of 1 microsecond.
    pub fn from_duration(now: Instant, duration: Duration) -> Self {
        let duration = duration.max(Duration::from_micros(1));
        Self {
            start: now,
            duration,
        }
    ...
    pub fn eager_from_duration(now: Instant, duration: Duration) -> Self {
        let duration = duration.max(Duration::from_micros(1));
    ...
    pub fn from_interval(now: Instant, interval: Interval) -> Self {
    ...
    ///
    /// This clamps the bounds of the timer to be able to fit in the interval field on the wire.
    pub fn eager_from_interval(now: Instant, interval: Interval) -> Self {
        Self::eager_from_duration(now, *interval)
    }

    /// Sets the tick duration of the timer. Clamped to a minimum of 1 microsecond.
    ...
    pub fn set_tlv_interval(&mut self, interval: Interval) {
        let interval = interval.max(Interval::from_duration(Duration::from_micros(1)));
...
mod test {
    ...
    fn new_eager_timer() {
        ...
        let mut eager = Timer::eager_from_duration(now, duration);
    ...
    fn regression_no_timer_can_have_zero_duration() {
        let timer = Timer::from_duration(Instant::now(), Duration::from_secs(0));
        assert_eq!(timer.duration, Duration::from_micros(1));
    }
}
